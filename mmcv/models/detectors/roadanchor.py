# ============================================================================
# roadanchor.py - detector for the road-adaptive anchoring axis.
#
#   detector mmcv/models/detectors/roadanchor.py                      RoadAnchor
#   head     mmcv/models/dense_heads/roadanchor_head.py               RoadAnchorHead
#   decoder  mmcv/models/modules/roadanchor/planning_decoder_score_yaw.py
#                                                          PlanningDecoderScoreYaw
#
#   The detector is a thin wrapper around the usual BEV two-stage pipeline: it
#   builds the BEV features, calls the head and converts the head outputs into
#   the result dicts used by the evaluation and the closed-loop driving agent.
#   All of the axis-specific wiring (score-ranked reference selection, score
#   injection into the planning decoder, heading regression) lives in the head.
#   The driving and open-loop evaluation paths read ego_fut_preds, which stays a
#   plain 2-channel position tensor; the heading prediction is only dumped for
#   visualisation and offline analysis and never reaches a controller.
# ============================================================================
import time
import copy
...
class RoadAnchor(MVXTwoStageDetector):
    """Detector for the road-adaptive anchoring axis.

    Perception (image backbone, BEV encoder, detection and map decoders) is
    identical to the upstream VAD-style detector; the planning behaviour is
    defined entirely by RoadAnchorHead.
    """
    def __init__(self,
                 use_grid_mask=False,
    ...
    def forward_pts_train(self,
        ...
        outs = self.pts_bbox_head(pts_feats, img_metas, prev_bev,
                                  ego_his_trajs=ego_his_trajs, ego_lcf_feat=ego_lcf_feat, ego_goal_points=ego_goal_points,
                                  gt_bboxes_list=gt_bboxes_3d, gt_labels_list=gt_labels_3d, gt_attr_labels=gt_attr_labels, 
                                  map_gt_bboxes_list=map_gt_bboxes_3d, map_gt_labels_list=map_gt_labels_3d)
        loss_inputs = [
    ...
    def forward_train(self,
        ...
        if 'ego_goal_points' in kwargs:
            ego_goal_points = kwargs['ego_goal_points']
        else:
            ego_goal_points = None
        ...
        losses_pts = self.forward_pts_train(img_feats, gt_bboxes_3d, gt_labels_3d,
                                            ...
                                            ego_lcf_feat=ego_lcf_feat, gt_attr_labels=gt_attr_labels, ego_goal_points=ego_goal_points)
    ...
    def simple_test(
        ...
        **kwargs
        ...
        new_prev_bev, bbox_pts, metric_dict = self.simple_test_pts(
            ...
            ego_lcf_feat=ego_lcf_feat,
            gt_attr_labels=gt_attr_labels,
            ego_goal_points=ego_goal_points
        )
    ...
    def simple_test_pts(
        ...
        for i, (bboxes, scores, labels, trajs, map_bboxes, \
            ...
            # When head.with_planning=False the planning keys of outs are None, so
            #   trajectory extraction is skipped and bbox_result gets no planning keys.
            if outs.get('ra_ego_trajs', None) is not None:
                ra_ego_trajs = outs['ra_ego_trajs'][i].cpu()
                ego_probability = outs['ego_probability'][i].cpu()
                lat, lon, pts, dim = ra_ego_trajs.shape

                ego_probability_bs = ego_probability.detach().reshape(-1)
                top_vals, top_idx = torch.topk(ego_probability_bs, k=1, largest=True, sorted=True)  # [1]
                top_idx = top_idx.squeeze(-1)

                ego_fut_preds = ra_ego_trajs.reshape(-1, pts, dim)  # [lat*lon, pts, dim]

                ego_fut_preds = ego_fut_preds[top_idx].unsqueeze(1).permute(1, 0, 2)  # [1, pts, dim]

                bbox_result['ego_fut_preds'] = ego_fut_preds
                bbox_result['ego_fut_cmd'] = ego_fut_cmd.cpu()

                # planning outputs kept for visualisation
                bbox_result['ra_ego_trajs'] = ra_ego_trajs
                bbox_result['ego_probability'] = ego_probability

            # ── Dump-only keys (the driving logic is unaffected) ──────────────
            #   The head already emits these through outs, but without forwarding them
            #   into bbox_result the closed-loop dumps carried neither the centerlines
            #   nor the reference scores.
            #   Ordering: ref_sel_idx comes from rank.topk(sorted=True), so all three
            #   keys are in descending score order, matching axis 0 (R) of cand_trajs
            #   (planning_decoder_score.py: loc.view(bs, R, num_mode, T, 2)).
            #   None guard: sel_score is None on the no-goal fallback.
            #   The heading prediction ra_ego_yaws is dumped as well.
            #   Driving (controller) and open-loop evaluation use only ego_fut_preds
            #   above; heading never enters a control path.
            for _dk in ('ra_centerlines', 'ra_centerlines_mask', 'ra_cl_sel_score',
                        'ra_ego_yaws'):
                _dv = outs.get(_dk, None)
                if _dv is not None:
                    bbox_result[_dk] = _dv[i].cpu()
        ...
        if gt_attr_labels is not None:
            ...
            with torch.no_grad():
                ...
                # ego planning metric
                # When there is no planning output (ego_fut_preds unset) the STP3
                #   planning metric is skipped. fut_valid_flag (and a plan_L2
                #   placeholder) are still kept in metric_dict so that the planning
                #   aggregation in dataset.evaluate() does not raise KeyError.
                if 'ego_fut_preds' in bbox_result:
                    ...
                    ego_fut_pred = ego_fut_preds[0] # mode 1
                    ...
                    for i_s in range(3):
                        metric_dict['plan_L2_{}s'.format(i_s + 1)] = 0.0
