...
def drop_path(x, drop_prob: float = 0.0, training: bool = False):
    ...
    # Generate random tensor in shape [batch_size, 1, 1, ...] to broadcast across feature dims
...
class PlutoTransformerEncoderLayer(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
...
class MapCrossAttentionLayer(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
        ...
        super().__init__()
        # Query normalization (for src1)
        self.norm_q = norm_layer(dim)
        
        # Key/Value normalization (for src2)
    ...
    def forward(
        ...
        """
        Args:
            src1: Query source (e.g., agent features) [B, N1, dim]
            src2: Key/Value source (e.g., map features) [B, N2, dim]
        Returns:
            Enhanced src1 [B, N1, dim]
        """
        q = self.norm_q(src1)
        kv = self.norm_kv(src2)
        ...
        src1 = src1 + self.drop_path1(attn_out)
        src1 = src1 + self.drop_path2(self.mlp(self.norm_ffn(src1)))
