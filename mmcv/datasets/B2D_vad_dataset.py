...
class B2D_VAD_Dataset(Custom3DDataset):
    ...
    def __init__(self, queue_length=4, bev_size=(200, 200),overlap_test=False,with_velocity=True,sample_interval=5,name_mapping= None,eval_cfg = None, map_root =None,map_file=None,past_frames=2, future_frames=6,point_cloud_range = [-51.2, -51.2, -5.0, 51.2, 51.2, 3.0] ,polyline_points_num=20,map_gt_dedup=False,map_gt_dedup_eps=0.5,*args, **kwargs):
        super().__init__(*args, **kwargs)
        # -- Map GT instance de-duplication (opt-in, default False) ----------
        #   The released *_HD_map.npz files store the (road_id, lane_id) list
        #   repeated an integer number of times, coordinates included. About a
        #   quarter of the lane groups appear two or three times, so roughly half
        #   of the per-frame GT is a copy of the same geometry. The Hungarian
        #   matcher is 1:1, so as many queries get assigned to the same element.
        #   With map_gt_dedup=True only one instance per (class, Chamfer < eps)
        #   group survives. eps is not a tuning knob: the distance distribution of
        #   duplicate pairs is bimodal (near 0 m, or above 1.37 m for genuinely
        #   distinct elements), so anything in 0.05-1.0 m gives the same result.
        self.map_gt_dedup = map_gt_dedup
        self.map_gt_dedup_eps = map_gt_dedup_eps
        self._map_gt_dedup_stat = [0, 0]      # [instances seen, instances removed] - for logging
        ...
        self.map_ann_file = 'data/infos/map_gt_anns.pkl'
        self.eval_cfg  = eval_cfg
    ...
    def get_data_info(self, index):
        ...
        # goal points (global route waypoints) used by the planning head
        far_global_route_point = info['command_far_xy'] # world coordinate
        near_global_route_point = info['command_near_xy'] # world coordinate
        
        far_global_route_point_lidar = info['sensors']['LIDAR_TOP']['world2lidar'] @ np.array([far_global_route_point[0], far_global_route_point[1], 0.0, 1.0]).reshape(4,1)
        near_global_route_point_lidar = info['sensors']['LIDAR_TOP']['world2lidar'] @ np.array([near_global_route_point[0], near_global_route_point[1], 0.0, 1.0]).reshape(4,1)

        ego_goal_points = np.concatenate([near_global_route_point_lidar[:2,0], far_global_route_point_lidar[:2,0]]).astype(np.float32).reshape(2,2) # (4,)
        
        input_dict['ego_goal_points'] = ego_goal_points
    ...
    def get_map_info(self, index):
        ...
        # -- Remove exact duplicate instances within a class (opt-in) --------
        #   With the default False this block is skipped entirely.
        if self.map_gt_dedup and len(polylines) > 1:
            polylines, gt_labels = self._dedup_map_gt(polylines, gt_labels)
    ...
    def _dedup_map_gt(self, polylines, gt_labels):
        """Keep one polyline per (class, Chamfer < eps) group, the first one seen.

        · The comparison uses the fixed_num (=polyline_points_num) uniformly
          resampled point sequence, the same resolution the model trains on.
        · Only instances of the same class are compared, so the real cost is a few
        tens of thousands of flops per frame. Order is preserved: the surviving
        instances keep their input order.
        """
        n = self.polyline_points_num
        pts = []
        for ls in polylines:
            d = np.linspace(0, ls.length, n)
            pts.append(np.array([[p.x, p.y] for p in (ls.interpolate(x) for x in d)]))
        labels = np.asarray(gt_labels)
        keep = np.ones(len(polylines), dtype=bool)
        eps = self.map_gt_dedup_eps
        for c in np.unique(labels):
            ...
            for ii in range(len(idx)):
                ...
                for jj in range(ii + 1, len(idx)):
                    ...
                    if not keep[b]:
                        continue
                    d = np.linalg.norm(pts[a][:, None, :] - pts[b][None, :, :], axis=-1)
                    if 0.5 * (d.min(1).mean() + d.min(0).mean()) < eps:
                        keep[b] = False
        self._map_gt_dedup_stat[0] += len(polylines)
        self._map_gt_dedup_stat[1] += int((~keep).sum())
        return ([polylines[i] for i in range(len(polylines)) if keep[i]],
                [gt_labels[i] for i in range(len(gt_labels)) if keep[i]])
    ...
    def _evaluate_single(self,
        ...
        # Generate GT annotations if not exists
        self._format_gt()
